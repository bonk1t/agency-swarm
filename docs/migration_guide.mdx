...
<Accordion title="Architectural & Persistence Details">
...
<Warning>
The steps below introduce **breaking changes** that must be implemented when migrating from v0.x to v1.x. Follow each step carefully to avoid runtime errors.
</Warning>
...
- `response_validator` parameter is completely removed from the Agent class
- Validation is now passed using `output_guardrails` and `input_guardrails`
</Step>

</Steps>

<Accordion title="Migration Reference Tables">
...
| `get_completion_stream()` | `get_response_stream()` (async) | **No longer supported** |
