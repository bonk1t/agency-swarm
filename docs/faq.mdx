...
```
Environment variables in a `.env` file are loaded automatically when `agency_swarm` is imported.
</Accordion>
...
agency = Agency(
    ...
    save_threads_callback=lambda new_threads: save_threads(new_threads, chat_id)
)
```
...
1. Use shared state inside your tools. Read more: [Agency Context](/additional-features/agency-context)
