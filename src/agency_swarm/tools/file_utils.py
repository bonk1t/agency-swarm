...
class AgentFileManager:
    ...
    def upload_file(self, file_path: str, include_in_vector_store: bool = True) -> str:
        ...
        try:
            ...
            logger.warning(
                f"Agent {self.agent.name}: Failed to rename file {fpath.name} to {destination_path}: {e}"
            )
        ...
        if self.agent._associated_vector_store_id and include_in_vector_store:
            try:
                ...
                try:
                    self.agent._openai_client_sync.vector_stores.retrieve(vector_store_id=self.agent._associated_vector_store_id)
                    logger.debug(
    ...
    def _add_code_interpreter_tool(self, code_interpreter_file_ids: list[str]):
        ...
        if not code_interpreter_tool_exists:
            logger.info(
                f"Agent {self.agent.name}: Adding CodeInterpreterTool"
            )
            self.agent.add_tool(CodeInterpreterTool(
